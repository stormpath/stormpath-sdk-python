...
def assert_instance(p_object, class_or_type_or_tuple, parameter_name):
    if not isinstance(p_object, class_or_type_or_tuple):
        full_name = class_or_type_or_tuple.__module__ + "." + \
            class_or_type_or_tuple.__name__
...
def is_default_port(parsed_url):
    """
    Returns True if the specified URI uses a standard port
    (i.e. http == 80 or https == 443), False otherwise.
